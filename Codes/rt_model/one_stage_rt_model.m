function model_est = one_stage_rt_model(data,rep_times,options,model_name,par_name,parfor_progress,save_csv,send_mail,pushbullet)
...
parfor i = 1:rep_times
    ...
    for j = 1:nid
        ...
        x0 = [normrnd(0,10,2,1); abs(normrnd(0,10,2,1))];
        [x(:,i,j),fval(:,i,j)] = one_stage_rt_nested(data(data.id == id(j),:),x0,[],[],[],[],lb,ub,[],options);
    end
end
[~,I] = min(fval,[],2);
