function model_est = one_step_decay_model(data,scale_array,col,stop,extra_col,rep_times,options,model_name,par_name,parfor_progress,save_csv,send_mail,pushbullet)
...
npar_os = length(col);
lb = [-Inf*ones(npar_os,1); 0];
ub = [Inf*ones(npar_os,1); 1];
x = zeros(npar_os+1, rep_times, n_id);
...
parfor i = 1:rep_times
    ...
    for j = 1:n_id
        ...
        x0 = [normrnd(0,5,npar_os,1); unifrnd(0,1)];
        try
            [x(:,i,j),fval(:,i,j)] = one_step_decay_nested(data{data.id==sub_id(j),:},scale_array,col,stop,ratio,cum_info,cum_info_val,info,draw,x0,[],[],[],[],lb,ub,[],options.default);
        catch
            warning('Current fmincon cannot continue. Using interior-point algorithm instead.');
            [x(:,i,j),fval(:,i,j)] = one_step_decay_nested(data{data.id==sub_id(j),:},scale_array,col,stop,ratio,cum_info,cum_info_val,info,draw,x0,[],[],[],[],lb,ub,[],options.backup);
        end
    end
end
[~,I] = min(fval,[],2);
...
x_reshaped = reshape(x, npar_os+1, []);
