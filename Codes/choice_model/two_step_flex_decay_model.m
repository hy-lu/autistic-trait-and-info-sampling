function model_est = two_step_flex_decay_model(data,scale_array,continue_to_second,os_col,ts_col,stop,extra_col,rep_times,options,model_name,par_name,parfor_progress,save_csv,send_mail,pushbullet)
...
nptr_ts = 2;
lb = [-Inf*ones(npar_ts+nptr_ts,1); 0];
ub = [Inf*ones(npar_ts+nptr_ts,1); 1];
...
parfor i = 1:rep_times
    ...
    for j = 1:n_id
        ...
        x0 = [normrnd(0,5,npar_ts+nptr_ts,1); unifrnd(0,1)];
        try
            [x(:,i,j),fval(:,i,j)] = two_step_flex_decay_nested(data{data.id==sub_id(j),:},scale_array,continue_to_second,os_col,ts_col,stop,ratio,cum_info,cum_info_val,info,draw,x0,[],[],[],[],lb,ub,[],options.default);
        catch
            warning('Current fmincon cannot continue. Using interior-point algorithm instead.');
            [x(:,i,j),fval(:,i,j)] = two_step_flex_decay_nested(data{data.id==sub_id(j),:},scale_array,continue_to_second,os_col,ts_col,stop,ratio,cum_info,cum_info_val,info,draw,x0,[],[],[],[],lb,ub,[],options.backup);
        end
    end
end
[~,I] = min(fval,[],2);
