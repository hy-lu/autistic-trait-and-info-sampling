            ...
            minlength = 5)
  {
    ...
    if (ci) {
      ...
      if (sym) {
        ...
        ncors <- prod(dim(r))
      if (adjust != "holm" & !(adjust %in% c("fdr", "BH"))) {
        dif.corrected <- qnorm(1 - alpha / (2 * ncors))
      }
      else if (adjust %in% c("fdr", "BH")) {
        ord <- order(abs(z), decreasing = TRUE)
        dif.corrected <- qnorm(1 - alpha * ord / (2 * ncors))
      } else {
      ...
      if (sym) {
        ...
        lower <- psych::fisherz2r(z - dif * sef)
        upper <- psych::fisherz2r(z + dif * sef)
        lower.corrected <- psych::fisherz2r(z - dif.corrected *
                                       sef)
        upper.corrected <- psych::fisherz2r(z + dif.corrected *
                                       sef)
        ci <- data.frame(
          lower = lower,
          r = r[lower.tri(r)],
        ...
        if (adjust != "holm" & !(adjust %in% c("fdr", "BH"))) {
          ...
          ord <- order(abs(z), decreasing = TRUE)
          dif.corrected <- qnorm(1 - (1 - alpha) * ord / (n.x * n.y))
        }
        else {
          ord <- order(abs(z), decreasing = FALSE)
        ...
        sef <- 1 / sqrt(n - 3)
        lower <- as.vector(psych::fisherz2r(z - dif * sef))
        upper <- as.vector(psych::fisherz2r(z + dif * sef))
        lower.corrected <- psych::fisherz2r(z - dif.corrected *
                                       sef)
        upper.corrected <- psych::fisherz2r(z + dif.corrected *
                                       sef)
        ...
        ci.adj <- data.frame(
          lower.adj = as.vector(lower.corrected),
          r = as.vector(r),
          upper.adj = as.vector(upper.corrected)
        )
